...
router.post("/productos", (req, res) => {
  let { id, nombre, descripcion, codigo, imagen, precio, stock } = req.body;
  let obj = {
    id,
    nombre,
    descripcion,
...
router.put("/productos/:id", (req, res) => {
  ...
  if (ident) {
      let identNumber = parseInt(ident)
    let { id, nombre, descripcion, codigo, imagen, precio, stock } = req.body;
    let obj = {
      id,
      nombre,
    ...
    const indice = array.findIndex((elemento) => {
        if (elemento.id === identNumber) {
          return true;
        }
      });
      array[indice] = obj
